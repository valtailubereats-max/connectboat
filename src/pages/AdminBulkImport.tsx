...
const AdminBulkImport: React.FC = () => {
  ...
  const handleStartBulkImportWithUrls = async (urlsToImport: string[]) => {
    ...
    const initialItems: BulkItem[] = uniqueUrls.map((url, idx) => {
      ...
      return {
        ...
        errorMessage: isDup ? 'Já existe um anúncio com este URL no marketplace.' : undefined,
        selected: !isDup,
        expanded: false
      };
    });
    ...
    for (let i = 0; i < initialItems.length; i++) {
      ...
      try {
        ...
        if (!resp.ok) {
          throw new Error(resData?.error || `[HTTP ${resp.status}] Falha ao importar URL.`);
        }
        ...
        if (resData.success && resData.data) {
          ...
          let matchedCategory = typeof d.category === 'string' ? d.category : 'Outros';
          ...
          setItems(prev => prev.map(item => item.id === currentItem.id ? {
            ...item,
            status: 'success',
            title: d.title || 'Anúncio Importado de ' + item.sourceSite,
            description: d.description || 'Descrição extraída de ' + item.url,
          ...
          throw new Error(resData.error || 'A extração via IA não retornou dados válidos.');
        }
      } catch (err: any) {
        failCount++;
    ...
    setActionStatus({
      type: 'success',
      message: `Extração em massa concluída! ${succCount} com sucesso, ${dupCount} duplicados ignorados, ${failCount} com falhas.`
    });
  };
  ...
  const handleBatchPublish = async (asDraft: boolean = false) => {
    ...
    if (selectedItems.length === 0) {
      setActionStatus({
        type: 'error',
        message: 'Selecione pelo menos um anúncio extraído com sucesso para publicar ou guardar rascunho.'
      });
      return;
    }
    ...
    setActionStatus({
      type: 'info',
      message: asDraft ? `A guardar ${selectedItems.length} rascunho(s)...` : `A publicar ${selectedItems.length} anúncio(s)...`
    });
    ...
    try {
      ...
      setActionStatus({
        type: 'success',
        message: asDraft
          ? `Sucesso! ${count} anúncio(s) guardado(s) como Rascunho com sucesso.`
          : `Sucesso! ${count} anúncio(s) publicado(s) no marketplace ConnectBoat com sucesso!`
      });
    } catch (err: any) {
      console.error('[AdminBulkImport] Batch publish error:', err);
      setActionStatus({
        type: 'error',
        message: `Erro ao publicar anúncios em lote: ${err.message || String(err)}`
      });
    } finally {
      setIsProcessing(false);
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-2 bg-indigo-500/20 text-indigo-300 px-3 py-1 rounded-full text-xs font-bold border border-indigo-400/30">
              <Layers size={14} /> Sistema de Importação em Massa
            </div>
            <h1 className="text-2xl md:text-3xl font-black tracking-tight text-white">
              Bulk Import Listings
            </h1>
            <p className="text-slate-300 text-xs md:text-sm font-medium max-w-2xl">
              Cole múltiplos URLs de marketplaces marinhos. O motor de IA extrairá automaticamente os títulos, imagens, preços e especificações náuticas de cada anúncio individualmente.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h3 className="font-extrabold text-slate-900 text-sm">
              Progresso do Lote ({currentIndex + 1} / {stats.total})
            </h3>
                ...
                <div className="p-4 md:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0 self-end md:self-auto">
                    {item.status === 'success' && (
                      ...
                      >
                        <Edit3 size={12} />
                        {item.expanded ? 'Ocultar Edição' : 'Editar'}
                  ...
                  <div className="p-4 md:p-6 border-t border-slate-100 bg-white space-y-4 text-xs font-sans">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                      <div className="sm:col-span-2">
                        <label className="font-bold text-slate-600 block mb-1">Título do Anúncio</label>
                        <input
                          type="text"
                          value={item.title || ''}
                      ...
                      <div>
                        <label className="font-bold text-slate-600 block mb-1">Fabricante</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="font-bold text-slate-600 block mb-1">Modelo</label>
                        <input
                          type="text"
                          value={item.model || ''}
                      ...
                      <div>
                        <label className="font-bold text-slate-600 block mb-1">Ano de Fabrico</label>
                        <input
                          type="text"
                          value={item.year || ''}
