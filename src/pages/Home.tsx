...
const Home = () => {
  ...
  const hireDragStartYRef = useRef<number | null>(null);
  const hireDragStartXRef = useRef<number | null>(null);
  const hireDragStartScrollYRef = useRef(0);
  const hireMouseDraggingRef = useRef(false);
  ...
  const handleHireMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
    if (e.button !== 0) return;
    hireMouseDraggingRef.current = true;
    hireDragStartYRef.current = e.clientY;
    hireDragStartXRef.current = e.clientX;
    hireDragStartScrollYRef.current = window.scrollY;
  };

  const handleHireMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!hireMouseDraggingRef.current || hireDragStartYRef.current === null || hireDragStartXRef.current === null) return;

    const deltaY = e.clientY - hireDragStartYRef.current;
    const deltaX = e.clientX - hireDragStartXRef.current;

    if (Math.abs(deltaY) > Math.abs(deltaX) && Math.abs(deltaY) > 3) {
      e.preventDefault();
      window.scrollTo({
        top: hireDragStartScrollYRef.current - deltaY,
        behavior: 'auto'
      });
    }
  };

  const handleHireMouseUp = () => {
    hireMouseDraggingRef.current = false;
    hireDragStartYRef.current = null;
    hireDragStartXRef.current = null;
  };

  const handleHireTouchStart = (e: React.TouchEvent<HTMLDivElement>) => {
    const touch = e.touches[0];
    if (!touch) return;
    hireDragStartYRef.current = touch.clientY;
    hireDragStartXRef.current = touch.clientX;
    hireDragStartScrollYRef.current = window.scrollY;
  };

  const handleHireTouchMove = (e: React.TouchEvent<HTMLDivElement>) => {
    const touch = e.touches[0];
    if (!touch || hireDragStartYRef.current === null || hireDragStartXRef.current === null) return;

    const deltaY = touch.clientY - hireDragStartYRef.current;
    const deltaX = touch.clientX - hireDragStartXRef.current;
            ...
            >
              ...
              <div className="relative z-10">
                ...
                {hireAds.length > 0 ? (
                  ...
                  <div
                    className="w-full overflow-x-auto overflow-y-visible scrollbar-none pb-1 pt-0.5 cursor-grab active:cursor-grabbing"
                    style={{
                      WebkitOverflowScrolling: 'touch',
                      touchAction: 'pan-x pan-y'
                    }}
                    onMouseDown={handleHireMouseDown}
                    onMouseMove={handleHireMouseMove}
                    onMouseUp={handleHireMouseUp}
                    onMouseLeave={handleHireMouseUp}
                    onTouchStart={handleHireTouchStart}
                    onTouchMove={handleHireTouchMove}
                    onTouchEnd={handleHireTouchEnd}
