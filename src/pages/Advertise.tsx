...
export default function Advertise() {
  ...
  const submitPreparedBanner = async () => {
    ...
    try {
      ...
      if (!prepareResponse.ok || prepareData?.success !== true || !prepareData.uploadUrl) throw new Error(prepareData?.errorMessage || prepareData?.error || 'Unable to prepare the carousel artwork upload.');
      const uploadResponse = await fetch(prepareData.uploadUrl, { method: 'PUT', headers: { 'Content-Type': sourceFile.type }, body: sourceFile });
      if (!uploadResponse.ok) throw new Error(`Carousel artwork upload failed (${uploadResponse.status}).`);
      ...
      if (!finalResponse.ok || finalData?.success !== true) throw new Error(finalData?.errorMessage || finalData?.error || 'Unable to submit your carousel artwork.');
      await loadOrder(); setSourceFile(null); if (sourceUrl) URL.revokeObjectURL(sourceUrl); setSourceUrl('');
    } catch (error: any) { setDesignError(error?.message || 'Unable to submit your carousel artwork.'); }
  ...
  return <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
    ...
    {!orderIdFromUrl ? <div className="mt-8 grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm p-5 sm:p-7 space-y-6">
        <div><h2 className="text-xl font-black text-slate-900">1. Choose your campaign</h2><p className="text-sm text-slate-500 mt-1">Longer display time gives your carousel artwork more attention in each rotation.</p></div>
