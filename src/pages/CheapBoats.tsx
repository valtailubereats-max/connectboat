...
const CheapBoats: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const snapshot = await getDocsWithCacheFallback(q, 'cheap_boats_page');
        if (!mounted) return;

        const list: Ad[] = [];
        snapshot.forEach((docSnap) => {
          const data = docSnap.data() as Ad;
          const isSaleBoat = data.listingIntent === 'sale' || data.category === 'Boats for Sale';
          const active = data.adStatus === 'active' || !data.adStatus;
          const ukListing = !data.country || data.country === 'United Kingdom' || data.country === 'Reino Unido';
          const price = Number(data.price ?? 0);

          if (isSaleBoat && active && !data.isHidden && ukListing && Number.isFinite(price) && price <= 1000) {
  ...
  const visibleAds = useMemo(() => {
    ...
    return ads.filter((ad) => {
      ...
      if (!term) return true;
      return [ad.title, ad.description, ad.city, ad.county, ad.manufacturer, ad.model]
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {FILTERS.map((item) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-bold transition ${filter === item.id ? 'bg-slate-900 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'}`}
              >
                {item.label}
              </button>
            ))}
        ...
        <div className="mb-5 mt-8 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">Listings shown here are currently approved on ConnectBoat and priced up to £1,000.</p>
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {[0, 1, 2].map((i) => <div key={i} className="h-80 animate-pulse rounded-2xl bg-slate-200" />)}
          </div>
        ) : visibleAds.length > 0 ? (
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
            ...
            <Link to="/" className="mt-5 inline-flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-3 text-sm font-bold text-white hover:bg-slate-800">
